...
data PostProcessSteps 
  ...
  -- config setting, AI_CONFIG_PP_CT_MAX_SMOOTHING_ANGLE, which allows you to
  -- specify a maximum smoothing angle for the algorithm. However, usually
  -- you'll want to leave it at the default value. Thanks.
  = CalcTangentSpace
  ...
  -- geometry, this step is compulsory or you'll just waste rendering time. If
  -- this flag is not specified, no vertices are referenced by more than one
  ...
  -- rendering. The aiProcess_ConvertToLeftHanded flag supersedes this setting
  -- and bundles all conversions typically required for D3D-based applications.
  | MakeLeftHanded
  -- | Triangulates all faces of all meshes.
  --
  -- By default the imported mesh data might contain faces with more than 3
  -- indices. For rendering you'll usually want all faces to be triangles. This
  -- post processing step splits up all higher faces to triangles. Line and
  -- point primitives are *not* modified!. If you want 'triangles only' with no
  -- other kinds of primitives, try the following solution:
  --
  -- Specify both aiProcess_Triangulate and aiProcess_SortByPType Ignore all
  ...
  -- option, AI_CONFIG_PP_RVC_FLAGS. This is quite useful if you don't need all
  -- parts of the output structure. Especially vertex colors are rarely used
  -- today ... . Calling this step to remove unrequired stuff from the pipeline
  -- as early as possible results in an increased performance and a better
  -- optimized output data structure. This step is also useful if you want to
  -- force Assimp to recompute normals or tangents. The corresponding steps
  -- don't recompute them if they're already there (loaded from the source
  -- asset). By using this step you can make sure they are NOT there.
  --
  ...
  -- AI_CONFIG_PP_GSN_MAX_SMOOTHING_ANGLE which allows you to specify an angle
  -- maximum for the normal smoothing algorithm. Normals exceeding this limit
  -- are not smoothed, resulting in a a 'hard' seam between two faces. Using a
  -- decent angle here (e.g. 80�) results in very good visual appearance.
  | GenSmoothNormals
  ...
  -- by the video driver/hardware. The maximum vertex buffer is usually
  -- limited, too. Both requirements can be met with this step: you may specify
  -- both a triangle and vertex limit for a single mesh.
  --
  ...
  --
  -- The output scene does still contain nodes, however, there is only a root
  -- node with children, each one referencing only one mesh, each mesh
  -- referencing one material. For rendering, you can simply render all meshes
  -- in order, you don't need to pay attention to local transformations and the
  -- node hierarchy. Animations are removed during this step. This step is
  ...
  -- first meshes's part of the normal list is zeroed. However, these artifacts
  -- are rare.
  -- Note:
  ...
  -- weight limit is 4 (defined as AI_LMW_MAX_WEIGHTS in aiConfig.h), but you
  -- can use the AI_CONFIG_PP_LBW_MAX_WEIGHTS setting to supply your own limit
  ...
  -- import fails. Importer::GetErrorString() or aiGetErrorString() carry the
  -- error message around.
  --
  -- Warning: There are some minor issues (e.g. 1000000 animation keyframes
  -- with the same time), but further postprocessing and use of the data
  -- structure is still safe. Warning details are written to the log file,
  -- AI_SCENE_FLAGS_VALIDATION_WARNING is set in aiScene::mFlags
  --
  ...
  -- miss ratio) for all meshes. The implementation runs in O(n) and is roughly
  -- based on the 'tipsify' algorithm (see this paper).
  --
  -- If you intend to render huge models in hardware, this step might be of
  -- interest for you. The AI_CONFIG_PP_ICL_PTCACHE_SIZEconfig setting can be
  ...
  -- join small meshes with equal characteristics, but they can't do their work
  -- if two meshes have different materials. Because several material settings
  -- are always lost during Assimp's import filters, (and because many
  ...
  -- pipeline (probably using *magic* material names), don't specify this flag.
  -- Alternatively take a look at the AI_CONFIG_PP_RRM_EXCLUDE_LIST setting.
  ...
  -- vertices without their normals. This works well for most objects, problems
  -- might occur with planar surfaces. However, the step tries to filter such
  -- cases. The step inverts all in-facing normals. Generally it is recommended
  -- to enable this step, although the result is not always correct.
  | FixInfacingNormals
  -- | This step splits meshes with more than one primitive type in homogeneous
  -- submeshes.
  --
  -- The step is executed after the triangulation step. After the step returns,
  -- just one bit is set in aiMesh::mPrimitiveTypes. This is especially useful
  -- for real-time rendering where point and line primitives are often ignored
  -- or rendered separately. You can use the AI_CONFIG_PP_SBP_REMOVE option to
  -- specify which primitive types you need. This can be used to easily exclude
  ...
  --
  -- (if you support lines&points for rendering but don't want the
  -- degenerates)
  -- 
  --   * Specify the aiProcess_FindDegenerates flag.
  --
  --   * Set the AI_CONFIG_PP_FD_REMOVE option to 1. This will cause the step
  --   to remove degenerated triangles from the import as soon as they're
  --   detected. They won't pass any further pipeline steps.
  -- 
  -- (if you don't support lines&points at all ...)
  --
  --   * Specify the aiProcess_FindDegenerates flag.
  --
  ...
  --   aiPrimitiveType_LINES@ to cause 'SortByPType' to reject point and line
  --   meshes from the scene.
  --
  -- Note:
  ...
  -- lines or points ... some exporters bypass the format specification and
  -- write them as degenerated triangle instead.
  | FindDegenerates
  ...
  -- recognizes this, they will be removed and can later be recomputed, i.e. by
  -- the aiProcess_GenSmoothNormals flag.
  --
  -- The step will also remove meshes that are infinitely small and reduce
  -- animation tracks consisting of hundreds if redundant keys to a single key.
  -- The AI_CONFIG_PP_FID_ANIM_ACCURACY config property decides the accuracy of
  ...
  -- to specify this step in every case. Note tha Assimp is not always able to
  -- match the original mapping implementation of the 3d app which produced a
  -- model perfectly. It's always better to let the father app compute the UV
  -- channels, at least 3ds max, maja, blender, lightwave, modo, ... are able
  -- to achieve this.
  --
  -- Note:
  ...
  -- processes all textures with transformed input UV coordinates and generates
  -- new (pretransformed) UV channel which replace the old channel. Most
  -- applications won't support UV transformations, so you will probably want
  -- to specify this step.
  -- 
  -- Note:
  ...
  -- currently not support per-node material assignment to meshes, which means
  -- that identical meshes with differnent materials are currently *not*
  ...
  --
  -- Nodes with no animations, bones, lights or cameras assigned are collapsed and joined.
  --
  -- Node names can be lost during this step. If you use special 'tag nodes' to
  -- pass additional information through your content pipeline, use the
  -- AI_CONFIG_PP_OG_EXCLUDE_LIST setting to specify a list of node names you
  -- want to be kept. Nodes matching one of the names in this list won't be
  -- touched or modified.
  --
  ...
  --
  -- Note:
  -- 'crappy' scenes with thousands of extremely small meshes packed in deeply
  -- nested nodes exist for almost all file formats. aiProcess_OptimizeMeshes
  -- in combination with aiProcess_OptimizeGraph usually fixes them all and
